...
// B - Brick, GG - Gold, P - Player, G - Ground, E - Exit
    ...
    public AbstractGameObject getObjectByCoordinates(int y, int x){
        try{
            return data[y][x];
    ...
    private List<AbstractMovableObject> movableObjects = new ArrayList<>();
        ...
        for (int i = 0; i < initialData.length; i++) {
            for (int j = 0; j < initialData[i].length; j++) {
               ...
               if (value.equals("B")) {
                   ...
                   data[i][j] = goldman;
                   movableObjects.add(goldman);
               } else if (value.equals("E")) {
                   data[i][j] = new Exit(this);
               } else if (value.equals("M")) {
                   data[i][j] = new Monster(this);
                   movableObjects.add((AbstractMovableObject)data[i][j]);
               }
    ...
    void runTheGame() throws Exception {
        for (int i = 0; i < 6; i++) {
            for (AbstractMovableObject a: movableObjects) {
                a.move(1);
            }
        }
    }
